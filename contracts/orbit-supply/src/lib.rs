...
impl OrbitSupply {
    ...
    fn calc_pending(env: &Env, user: &Address) -> (i128, u64, u64) {
        ...
        let pending = daily.checked_mul(periods as i128).expect("overflow");
...
mod test {
    #[test]
    fn reward_constants_are_sane() {
        // Host/unit smoke (no soroban-env-host). Wasm contract is validated in CI via
        // cargo fmt + clippy + build --target wasm32v1-none.
        assert!(super::PERIOD_SECS > 0);
        assert!(super::REWARD_PER_MILLION_STROOPS > 0);
        assert!(super::MILLION > 0);
    }
}
