...
Prediction markets and perpetuals are **fully on-chain**. Stakes and margin are held in Soroban contracts; Freighter signs contract invocations. There is no off-chain balance sheet.
...
Restart the API server. Chat flows (`invest 2 XLM on Brazil to win`, `open a 200 USDC long on bitcoin at 5x`) build **contract invocations**; Freighter signs; tokens move **into the contracts**.

## User token approvals

`place_bet` / `open_position` pull tokens via SAC `transfer` from the user. The user must have a trustline/balance for USDC SAC when opening perps. XLM uses the native SAC above.
