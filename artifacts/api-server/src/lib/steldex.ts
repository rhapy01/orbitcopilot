...
export async function matchSteldexAddLiquidityAmounts(input: {
 ...
 const dec1 = steldexDecimals(input.symbol1);

 try {
 // Probe ~1 unit of token0 → token1 for spot ratio (CLMM full-range ≈ spot)
 const quote = await getSteldexSwapQuote({
 walletAddress: STELDEX_QUOTE_WALLET,
 fromTokenContract: input.token0Contract,
 toTokenContract: input.token1Contract,
 amountIn: toSteldexUnits("1", dec0),
 slippageBps: 100,
 });
 ...
 if (human != null && human !== "") {
 outHuman = parseFloat(String(human));
 } else {
 const raw =
 (quote as any)?.amountOutRaw ??
 (quote as any)?.amountOut ??
 (quote as any)?.minAmountOut ??
 (quote as any)?.minAmountOutRaw;
 if (raw != null && raw !== "") {
 outHuman = parseFloat(fromSteldexUnits(String(raw), dec1));
 }
 }

 if (outHuman == null || !Number.isFinite(outHuman) || outHuman <= 0) {
 logger.warn({ quote }, "StelDex LP ratio quote missing amountOut");
 return null;
 }
