...
/** Format a human LP amount for chat/cards (trim trailing zeros). */
export function formatLpAmount(n: number, maxDecimals = 7): string {
 if (!Number.isFinite(n) || n <= 0) return "0";
 const d = Math.min(Math.max(0, maxDecimals), 8);
 let s = n.toFixed(d);
 if (s.includes(".")) s = s.replace(/\.?0+$/, "");
 return s === "" || s === "-0" ? "0" : s;
}

export type SizedLpAmounts = {
 amount0: number;
 amount1: number;
};
