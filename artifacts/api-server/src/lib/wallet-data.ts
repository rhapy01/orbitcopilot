...
/** Canonical display key: cUSDC ≡ USDC (same Circle SAC / trustline). pUSDC stays separate. */
...
function displayAsset(code: string): string {
  return assetKey(code) === "USDC" ? "USDC" : assetKey(code);
}
...
/** Structured wallet balances for LLM tools (Horizon classic + StelDex Soroban tokens). */
export async function fetchWalletBalances(
  publicKey: string,
  opts?: { asset?: string; hoursAgo?: number }
): Promise<WalletBalancesPayload> {
  const [rows, steldex] = await Promise.all([
