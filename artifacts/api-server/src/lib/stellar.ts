...
export function trustlineSymbolsForAction(action: {
 ...
 // Live Blend UI pool uses Circle USDC SAC - classic trustline still applies for USDC/CETES/TESOURO.
 if (type.startsWith("blend_")) {
 if (
 action.sendAsset &&
 ["USDC", "CETES", "TESOURO", "EURC"].includes(String(action.sendAsset).toUpperCase())
 ) {
 return [action.sendAsset];
 }
 return [];
 }
...
export async function submitSignedTransaction(
 ...
 let message = extras ? JSON.stringify(extras) : err?.message ?? "Transaction submission failed";
 // Horizon often returns { "transaction": "tx_too_late" }
 const txCode =
 typeof extras?.transaction === "string" ? extras.transaction : "";
 if (txCode === "tx_too_late" || /tx_too_late/i.test(message)) {
 message =
 "This transaction expired (time bounds). Refreshing a fresh one - try again.";
 }
