import { formatBlendMarkets, getBlendContracts } from "./blend";
...
export async function getLiveDefiOpportunities(): Promise<LiveOpportunity[]> {
  ...
  try {
    ...
    for (const symbol of ["XLM", "USDC", "BLND"]) {
      if (!contracts.ids[symbol]) continue;
      out.push({
        ...
        tvlUsd: 0,
        riskLevel: "medium",
        description: `Supply ${symbol} on Blend TestnetV2 pool ${contracts.ids.TestnetV2}`,
        minDeposit: 0,
        rewards: ["BLND"],
      });
    }
