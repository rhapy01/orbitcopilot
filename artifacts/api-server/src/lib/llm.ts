...
const TOOLS = [
 ...
 lockWeeks: { type: "number", description: "StelDex farm lock duration in weeks (1–156). Required for steldex_stake. Never invent a default — ask the user how many weeks if they did not say." },
...
export async function runLlmCopilot(
 ...
 "3. YIELD FARMING: take the LP tokens you received from liquidity provision and stake them in a farm to earn STELLAR rewards. This requires holding LP tokens first. Action type: steldex_stake. ALWAYS require lockWeeks from the user (1–156). If they say \"stake XLM/cUSDC\" without weeks, ASK how many weeks — never default to 52.",
 ...
 "Never call get_portfolio for a single-asset balance question.",
  "For on-chain actions (swap/LP/farm/lend/borrow/send/predict/perp/nft/orbit-supply/defindex/meridian/bridge) → call propose_action with amounts and asset codes. For LP, amountB may be AUTO.",
 "INTENT AUTOCORRECT: Never dead-end on typos or near-miss protocol names. Map defiindex/defi-index/de findex → DeFindex (defindex_deposit/withdraw). Map blennd → Blend, stelex → StelDex, soro swap → Soroswap. If the user clearly wants a vault deposit (\"deposit 10 XLM to defiindex\"), propose_action defindex_deposit — do NOT say the protocol is unrecognized.",
 "DeFindex vaults (testnet): defindex_deposit / defindex_withdraw for XLM, USDC (Blend USDC), or CETES. Example: deposit 10 XLM into defindex.",
 "Meridian: meridian_deposit / meridian_withdraw for Blend USDC vault.",
 "CCTP bridge (USDC only, Stellar Testnet → Base/Eth/Arb/OP/Arc): cctp_bridge. sendAmount=USDC amount, destination=0x EVM address, destAsset=base|ethereum|arbitrum|optimism|arc (or marketHint). Example: bridge 1 USDC to arc 0x…. Do NOT say Orbit cannot bridge when they ask to bridge USDC via CCTP/Circle.",
 "Never invent that a listed protocol is \"not recognized\". Integrated: Horizon, StelDex, Soroswap, Aquarius, Phoenix, Blend, DeFindex, Meridian, Reflector, Friendbot, Orbit Predict/Perps/NFT/Token Launch/Supply, Circle CCTP.",
 "If the user wants a transaction but omitted the amount (e.g. \"swap XLM to USDC\", \"faucet USDC\", \"supply USDC on blend\"), ask how much to use — do NOT lecture about Friendbot. Friendbot already funds XLM on wallet create. For \"faucet USDC\" ask how many XLM to swap to USDC. Exception: for add_liquidity with a numeric amount but missing pair, ask only for the pair asset — never a second amount. For steldex_stake without weeks, ask how many weeks to lock (do not assume 52).",
 ...
 "NFTs (SEP-50): nft_create_collection supports public mint price, allowlist/presale price, and per-wallet limits (SeaDrop-style). After deploy: open public mint / open allowlist mint, set mint price, add wallets to allowlist, mint status. Secondary: list/buy (floor = lowest listing). Also nft_mint / nft_list / nft_buy / nft_transfer / nft_cancel. For unique drops: upload media pack (ZIP) then mint next NFT. Tokens: token_deploy / token_mint (classic asset + SAC).",
