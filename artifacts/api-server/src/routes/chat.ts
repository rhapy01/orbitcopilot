...
async function parseSteldexIntents(
  ...
  if (addLiqMatch) {
    ...
    if (pool) {
      ...
      return {
        kind: "action",
        text: `Ready to add ${amount0} ${pool.symbol0} and ${amount1} ${pool.symbol1} to ${pool.pair} on StelDex (full-range). Sign each step with your connected wallet.`,
        action: {
    ...
    if (soroswapConfigured() && (await isSoroswapPair(symbolA, symbolB))) {
      return {
        kind: "action",
        text: `Soroswap add liquidity: ${amountA} ${symbolA.toUpperCase()} + ${amountB} ${symbolB.toUpperCase()}. Sign with your connected wallet.`,
        action: {
...
async function getDeterministicResponse(
  ...
  // Multi-swap: "swap 200 XLM to pUSDC, cUSDC, EURC each"
  {
    ...
    if (multi) {
      ...
      return {
        text: `Prepared ${actions.length} swaps (${multi.amount} ${multi.fromAsset} each → ${multi.toAssets.join(", ")}). Sign each card.\n${notes.join("\n")}`,
  ...
  if (lower.includes("blend")) {
    ...
    if (blendOp) {
      ...
      return {
        text: `Blend ${type.replace("blend_", "")}: ${amount} ${reserve.symbol} on TestnetV2. Review and sign with your connected wallet.`,
        action: {
          type,
          requestType,
          sendAmount: amount,
  ...
  if (faucetMatch) {
    ...
    try {
      ...
      const tip =
        symbol === "USDC"
          ? " USDC is used for Orbit Perps margin and Blend — try \"open a 200 USDC long on BTC at 5x\" after it arrives."
          : "";
      return {
      ...
      if (llm && (llm.text || llm.action || llm.actions?.length)) {
        ...
        if (actions?.length) {
          ...
          for (const a of actions) {
            const { action: gated, trustlineText } = await enrichChatActionWithTrustline(
